...
class NpyPatchListDataset(Dataset):
    ...
    def __init__(self, list_path: str | Path, *, split: str='train', val_every: int=500):
        ...
        if split == 'train':
            ...
            self.paths = [paths[i] for i in range(n) if i % val_every == 0]
        if not self.paths:
            raise RuntimeError(f'Empty split after filtering: split={split}, total={n}. Try smaller val_every (now {val_every}).')
