...
def patch_manhattan_distance_matrix(grid_t: int, grid_h: int, grid_w: int, *, device=None, dtype=torch.float32) -> torch.Tensor:
...
class GeomBiasTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, nhead: int, dim_feedforward: int, *, dropout: float=0.1, activation: str='gelu', layer_norm_eps: float=1e-05, norm_first: bool=True):
        super().__init__()
        if d_model % nhead != 0:
