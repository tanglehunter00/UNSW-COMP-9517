...
def train_one_epoch(model: MaskedAutoencoder2D, loader: DataLoader, optimizer: torch.optim.Optimizer, device: torch.device, mask_ratio: float) -> float:
    model.train()
    total = 0.0
    n = 0
...
@torch.no_grad()
def eval_epoch(model: MaskedAutoencoder2D, loader: DataLoader, device: torch.device, mask_ratio: float) -> float:
    model.eval()
    total = 0.0
    n = 0
    for batch in loader:
...
def parse_args():
    p = argparse.ArgumentParser(description='EWS 2D MAE pretraining')
    ...
    p.add_argument('--device', type=str, default='', help='Default: require CUDA; pass cpu for CPU (debug)')
    return p.parse_args()

def run_training(*, list_path: str, ckpt_dir: str, img_size: int=350, patch_size: int=25, epochs: int=10, batch_size: int=8, lr: float=0.00015, weight_decay: float=0.05, mask_ratio: float=0.5, num_workers: int=0, val_every: int=500, device: str='') -> None:
...
def _run_from_args(args):
    ...
    model = MaskedAutoencoder2D(img_size=args.img_size, patch_size=args.patch_size).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        path = ckpt_dir / f'mae2d_epoch{epoch}.pt'
        torch.save({'model': model.state_dict(), 'epoch': epoch, 'args': vars(args)}, path)
        if va < best:
            best = va
            torch.save({'model': model.state_dict(), 'epoch': epoch, 'args': vars(args)}, ckpt_dir / 'mae2d_best.pt')
