"""EWS 2D MAE pretrain entry (standalone from repo root `pretrain/` package)."""
...
_CUDA_INSTALL_HINT = (
    "GPU training required but torch.cuda.is_available() is False.\n"
    "Common causes: 1) CPU-only PyTorch (default pip install torch); "
    "2) Driver / CUDA runtime issues.\n"
...
def resolve_device(device_str: str) -> torch.device:
    """Default: require CUDA; allow CPU only if device_str is cpu."""
...
def run_training(
    *,
    ...
    """Notebook / script API; same logic as main()."""
    class A:
        pass
