...
from geom_attn_bias import GeomBiasTransformerEncoder, GeomBiasTransformerEncoderLayer, dformerv2_style_head_decays, patch_manhattan_distance_matrix
...
class MaskedAutoencoder2DGeom(nn.Module):
    ...
    def __init__(self, img_size: int=350, patch_size: int=25, in_chans: int=3, embed_dim: int=384, depth: int=6, num_heads: int=6, decoder_embed_dim: int=256, decoder_depth: int=4, decoder_num_heads: int=8, mlp_ratio: float=4.0, *, grid_t: int=1, use_geom_bias: bool=True, dropout: float=0.1):
        ...
        enc_layers = [GeomBiasTransformerEncoderLayer(embed_dim, num_heads, int(embed_dim * mlp_ratio), dropout=dropout, norm_first=True) for _ in range(depth)]
        ...
        dec_layers = [GeomBiasTransformerEncoderLayer(decoder_embed_dim, decoder_num_heads, int(decoder_embed_dim * mlp_ratio), dropout=dropout, norm_first=True) for _ in range(decoder_depth)]
