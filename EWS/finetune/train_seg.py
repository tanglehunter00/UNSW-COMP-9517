...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save({'model': model.state_dict(), 'epoch': epoch, 'args': vars(args), 'val_dice': va_dice, 'val_acc': va_acc, 'val_auc': va_auc}, out_dir / f'unet_mae_epoch{epoch}.pt')
        if va_dice > best_dice:
            best_dice = va_dice
            torch.save({'model': model.state_dict(), 'epoch': epoch, 'args': vars(args), 'val_dice': va_dice, 'val_acc': va_acc, 'val_auc': va_auc}, best_path)
