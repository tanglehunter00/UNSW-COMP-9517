...
class MAEEncoderUNetDecoder(nn.Module):

    def __init__(self, mae: MaskedAutoencoder2D, *, num_classes: int=1, base_ch: int=48):
        super().__init__()
        self.mae = mae
        C = base_ch
...
def build_model_from_mae_ckpt(ckpt_path: str, *, img_size: int=350, patch_size: int=25, num_classes: int=1, freeze_mae: bool=False) -> MAEEncoderUNetDecoder:
    ckpt = torch.load(ckpt_path, map_location='cpu', weights_only=False)
    mae = MaskedAutoencoder2D(img_size=img_size, patch_size=patch_size)
    mae.load_state_dict(ckpt['model'], strict=True)
    model = MAEEncoderUNetDecoder(mae, num_classes=num_classes)
    if freeze_mae:
        for p in model.mae.parameters():
            p.requires_grad = False
    return model

def build_model_random_mae(*, img_size: int=350, patch_size: int=25, num_classes: int=1, freeze_mae: bool=False) -> MAEEncoderUNetDecoder:
    mae = MaskedAutoencoder2D(img_size=img_size, patch_size=patch_size)
    model = MAEEncoderUNetDecoder(mae, num_classes=num_classes)
    if freeze_mae:
        for p in model.mae.parameters():
