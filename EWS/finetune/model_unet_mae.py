"""MAE encoder + UNet-style decoder (multi-scale skips via interpolation) for EWS segmentation."""
...
class MAEEncoderUNetDecoder(nn.Module):
    """
    Encoder: pretrained MAE patch_embed + TransformerEncoder → (B, D, g, g).
    Shallow CNN skips; bottleneck fuses with MAE features then decodes to full res.
    """

    def __init__(
        self,
        mae: MaskedAutoencoder2D,
        *,
        num_classes: int = 1,
        base_ch: int = 48,
    ):
...
def build_model_random_mae(
    *,
    ...
    """Random MAE init (no pretrained weights); ablation or decoder-only training."""
    mae = MaskedAutoencoder2D(img_size=img_size, patch_size=patch_size)
    model = MAEEncoderUNetDecoder(mae, num_classes=num_classes)
    if freeze_mae:
        for p in model.mae.parameters():
