"""EWS segmentation finetune: geometric MAE encoder + UNet decoder (parallel to train_seg.py)."""
...
SEGMENTATION_TRAIN_LOG_CSV = _FIN / "segmentation_training_log_geom.csv"
sys.path.insert(0, str(_FIN))
sys.path.insert(0, str(_PRETRAIN))

from dataset_ews_seg import EWSSegmentationDataset  # noqa: E402
from model_unet_mae_geom import build_model_from_mae_ckpt  # noqa: E402
...
def reset_segmentation_train_log_csv(path: Path | None = None) -> Path:
    """Overwrite CSV with header only; default: EWS/finetune/segmentation_training_log_geom.csv."""
...
def main():
    p = argparse.ArgumentParser(description="Finetune geom-MAE + UNet (use mae2d_geom_best.pt)")
    p.add_argument(
        "--dataset_root",
        type=str,
        default="",
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="mae2d_geom_best.pt or geom epoch checkpoint",
    )
    p.add_argument(
        "--out_dir",
        type=str,
        default="",
        help="Where to save finetuned weights; default finetune_seg_ckpts_geom",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Metrics CSV (default: EWS/finetune/segmentation_training_log_geom.csv)",
    )
    args = p.parse_args()
    ...
    out_dir = Path(args.out_dir) if args.out_dir else repo_ews / "data" / "finetune_seg_ckpts_geom"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(
            ...
            out_dir / f"unet_mae_geom_epoch{epoch}.pt",
        )
