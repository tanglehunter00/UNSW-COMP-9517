...
class MAEGeomEncoderUNetDecoder(nn.Module):
    ...
    def __init__(self, mae: MaskedAutoencoder2DGeom, *, num_classes: int=1, base_ch: int=48):
        super().__init__()
        self.mae = mae
        C = base_ch
        self.stem = nn.Sequential(nn.Conv2d(3, C, 3, padding=1, bias=False), nn.GroupNorm(min(8, C), C), nn.GELU())
...
def build_model_from_mae_ckpt(ckpt_path: str, *, img_size: int=350, patch_size: int=25, num_classes: int=1, freeze_mae: bool=False) -> MAEGeomEncoderUNetDecoder:
    ckpt = torch.load(ckpt_path, map_location='cpu', weights_only=False)
    use_geom = bool(ckpt.get('use_geom_bias', True))
    mae = MaskedAutoencoder2DGeom(img_size=img_size, patch_size=patch_size, use_geom_bias=use_geom)
...
def build_model_random_mae(*, img_size: int=350, patch_size: int=25, num_classes: int=1, freeze_mae: bool=False, use_geom_bias: bool=True) -> MAEGeomEncoderUNetDecoder:
    mae = MaskedAutoencoder2DGeom(img_size=img_size, patch_size=patch_size, use_geom_bias=use_geom_bias)
    model = MAEGeomEncoderUNetDecoder(mae, num_classes=num_classes)
    if freeze_mae:
        for p in model.mae.parameters():
            p.requires_grad = False
